...
async function accessKeys(env: Env): Promise<CryptoKey[]> {
  ...
  if (accessKeyCache && Date.now() - accessKeyCache.at < 3_600_000)
...
async function verifyAccess(request: Request, env: Env): Promise<boolean> {
  if (
    ...
    throw new ApiError(
      "Cloudflare Worker 还没有配置 ACCESS_TEAM_DOMAIN 和 ACCESS_AUD",
      503,
    );
  }
...
async function githubContent(
  id: string,
  env: Env,
): Promise<{ file: GitHubContent; artwork: Record<string, unknown> }> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      throw new ApiError(
        "GITHUB_TOKEN 没有读取仓库 Contents 的权限",
        502,
        detail,
      );
    }
...
function adminArtwork(artwork: Record<string, unknown>) {
  ...
  const mediaItems = Array.isArray(artwork.media)
    ? artwork.media.filter(isRecord)
    : [];
  const selectedMedia =
    mediaItems.find((item) => Number(item.index) === displayImageIndex) ??
    mediaItems[0];
  ...
  return {
    ...
    status,
    deleted_at:
      typeof artwork.deleted_at === "string" ? artwork.deleted_at : undefined,
    ...
    overrides: {
      ...
      ...(overrideAuthorName !== undefined
        ? { author_name: overrideAuthorName }
        : {}),
    },
    effective: {
...
async function listAdminArtworks(env: Env): Promise<Response> {
  ...
  if (!Array.isArray(entries))
    throw new ApiError("GitHub 返回了无法识别的藏品目录", 502);
  const ids = entries
  ...
  for (let index = 0; index < ids.length; index += 5) {
    const batch = await Promise.all(
      ids
        .slice(index, index + 5)
        .map(async (id) =>
          adminArtwork((await githubContent(id, env)).artwork),
        ),
    );
    artworks.push(...batch);
  }
...
async function commitArtwork(
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      throw new ApiError(
        "GITHUB_TOKEN 没有仓库 Contents 写入权限",
        502,
        detail,
      );
    }
...
function validateIngest(input: IngestRequest): {
  ...
  if (
    parsed.source === "other" &&
    (!input.image_urls || !input.title || !input.author_name)
  ) {
    throw new ApiError(
      "任意网站目前还需要 image_urls、title 和 author_name；Pixiv 与 X 只需分享作品链接",
    );
  }
...
async function dispatchIngest(
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      throw new ApiError(
        "GITHUB_TOKEN 没有启动 GitHub Actions 的权限",
        502,
        detail,
      );
    }
    throw new ApiError("无法启动 GitHub 采集任务", 502, {
      status: response.status,
      detail,
    });
  }
...
async function updateArtwork(
  ...
  if (!input.overrides && !input.status)
    throw new ApiError("没有需要保存的修改");
  if (
    input.overrides &&
  ...
  const current =
    typeof artwork.overrides === "object" &&
    artwork.overrides !== null &&
    !Array.isArray(artwork.overrides)
  ...
  if (input.status) {
    if (!isArtworkStatus(input.status)) throw new ApiError("藏品状态不正确");
    artwork.status = input.status;
    if (input.status === "deleted")
      artwork.deleted_at = new Date().toISOString();
    else delete artwork.deleted_at;
  }

  const action = input.status ? `set ${id} ${input.status}` : `edit ${id}`;
  const committed = await commitArtwork(
    id,
    file,
    artwork,
    `content: ${action}`,
...
async function reingestArtwork(request: Request, env: Env): Promise<Response> {
  const input = await readJson<{ id?: string; display_image?: number }>(
    request,
  );
  ...
  return dispatchIngest(
    {
      url: source.url,
      display_image:
        input.display_image ?? Number(artwork.display_image_index ?? 1),
      force: true,
    },
    env,
  );
  ...
  if (url.pathname.startsWith("/api/admin/")) {
    ...
    if (url.pathname === "/api/admin/session") {
      if (request.method !== "GET")
        throw new ApiError("不支持这种请求方式", 405);
    ...
    const deploymentMatch = url.pathname.match(
      /^\/api\/admin\/deployments\/([a-f0-9]{40})$/,
    );
    if (deploymentMatch) {
      if (request.method !== "GET")
        throw new ApiError("不支持这种请求方式", 405);
